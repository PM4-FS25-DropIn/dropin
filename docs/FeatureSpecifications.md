# DropIn Feature Specifications

This document outlines the core and potential features of DropIn, providing a structured breakdown of their functionality, purpose, and expected behavior.

## 🌟 Core Features

### 1️⃣ EventMap
**📍 Description:** A real-time, interactive map displaying nearby DropIn events.
**✅ Key Functionality:**
*   Users can browse events happening around them.
*   Events are color-coded based on type and status (ongoing, upcoming, full, etc.).
*   Clicking on an event provides more details and an option to join.

### 2️⃣ Creating, Joining, and Managing Events
**📝 Description:** Users can create, join, and manage events seamlessly.
**✅ Key Functionality:**
*   Quick event creation with title, description, location, and time.
*   Host controls: Limit participants, close events, edit event details.
*   Simple join and leave functionality for attendees.

### 3️⃣ Temporary Group Chats
**💬 Description:** Each event has an auto-generated group chat that expires when the event ends.
**✅ Key Functionality:**
*   Real-time messaging between attendees.
*   Media sharing (if enabled in the future).
*   Chat auto-deletes after event completion.

### 4️⃣ Finding Events Nearby
**🔍 Description:** Helps users discover events based on proximity and preferences.
**✅ Key Functionality:**
*   Events filtered by categories, location, and time.

### 5️⃣ Notification System
**🔔 Description:** Keeps users updated on relevant events and activities.
**✅ Key Functionality:**
*   Push notifications for nearby events, invitations, and updates.
*   Customizable notification preferences.

### 6️⃣ Event Rating + Feedback
**⭐ Description:** Users can rate events and provide feedback.
**✅ Key Functionality:**
*   Star rating system for past events.
*   Option to leave comments for the host.
*   Helps improve event quality.

### 7️⃣ DropIn (24h) + DropSoon (up to 7 days in advance) Events
**⏳ Description:** Two event types for different planning needs.
**✅ Key Functionality:**
*   DropIn Events: Spontaneous, expire in 24 hours.
*   DropSoon Events: Can be planned up to 7 days in advance.

## 🚀 Future Enhancements (If Time Allows)
*"Nice-to-Have" Features for Extended Functionality*

### 🌐 DropIn Network
*   Option to add friends and get notified when they attend or create events.
*   Friend activity feed (privacy settings included).

### 📸 Event Photo Sharing
*   Users can upload and share photos within the event chat.
*   Hosts can enable/disable this feature.

### 🆔 Personal & Business Accounts
*   Business accounts can create promotional or ticketed events.
*   Personal accounts remain casual and social.

### 💎 Pro Subscription for More Features
*   Premium users get access to exclusive event filters, analytics, and priority event visibility.
*   Subscription model for monetization.

## 📌 Notes
*   Core features are prioritized for MVP.
*   Future enhancements will be assessed based on demand and feasibility.

**🚀 Stay tuned for updates!**